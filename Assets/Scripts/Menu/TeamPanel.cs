...
public class TeamPanel : MonoBehaviour
{
    ...
    public void SetDragTargets(List<Transform> cards, float dropDistance)
    {
        _footballerCards.ForEach((card) => {
            card.SetDragTargets(cards, dropDistance);
        });
        SetBattleMode();
    }
    ...
    public void SetTeam(List<Footballer> team)
    {
        _footballerCards.ForEach(card => {
            if (card != null)
                Destroy(card.gameObject);
        ...
        foreach (var footballer in team)
        {
            var newCard = Instantiate(_cardPrefab, _content);
            newCard.Init(OnShowCardPanel, OnCardDestroy, footballer);
            _footballerCards.Add(newCard);
